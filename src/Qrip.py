...
class QripApp:
    ...
    def _run_download(self, url, quality):
        ...
        while True:
            ...
            while "\n" in buf:
                line, buf = buf.split("\n", 1)
                clean = self._sanitize_terminal_line(line)
                if clean:
    ...
    def _sanitize_terminal_line(self, raw_line):
        # Remove generic ANSI escape sequences (colors, cursor movement, etc.)
        clean = re.sub(r"\x1B(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])", "", raw_line)
        # Remove OSC / terminal-title sequences if present
        clean = re.sub(r"\x1b\][^\x07]*(?:\x07|\x1b\\)", "", clean)
        # Keep only printable characters + tab
